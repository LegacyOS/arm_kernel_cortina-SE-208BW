...
void i2s_init(void)
{
	...
	WRITE_SSP_REG(SSP_RX_SLOT_VALID0, 0x00000005);		
  	...
  	//WRITE_SSP_REG(SSP_CTRL_STATUS, 0xC00081FF);//0x9F100000
  	//mdelay(250);
#if (HAVE_CS4341==1)	  				
  	WRITE_SSP_REG(SSP_CTRL_STATUS, 0xC00001FF);//0x80100000
#els
	WRITE_SSP_REG(SSP_CTRL_STATUS, 0x000001FF);//0x80100000
#endif					
...
static int ssp_i2s_dsp_open(struct inode *inode, struct file *file)
{
	...
	data = READ_DMA_REG(DMA_CHEN);
        if (data == 0)
		WRITE_DMA_REG(DMA_CFG, 0x0); //disable DMA
	WRITE_DMA_REG(DMA_CH2_CSR, 0x0); //disable channel
...
static int ssp_i2s_dsp_release(struct inode *inode, struct file *file)
{
	unsigned int data;
        ...
        //disable dma int
        i2s_init(); //for cat
        *((volatile unsigned int *)IRQ_MASK(IO_ADDRESS(SL2312_INTERRUPT_BASE))) &= ~(IRQ_DMA_MASK);
	data = READ_DMA_REG(DMA_CHEN);
        if (data == 0)
       		 WRITE_DMA_REG(DMA_CFG, 0x0); //disable DMA
        WRITE_DMA_REG(DMA_CH2_CSR, 0x0); //disable channel
...
static ssize_t ssp_i2s_dsp_write(struct file *file_p, const char __user *buf, size_t count, loff_t * ppos)
{
	...
	int ofs=0, written = 0, len=0, j=0,proc_w=0, in_cont=0,in_bufok=0,	wt_curr=0,data_format=0;
	...
	if(ofs > 0)
		written++;
	//printk("proc_w:%d loop:%d ofs:%d \n",proc_w,written,ofs);

	while (proc_w)
	{
		spin_lock_irqsave(&ssp_lock,flags);
		in_cont = ssp_i2s.in_cont;
		spin_unlock_irqrestore(&ssp_lock, flags);
			
		if ((in_cont > (LLP_SIZE-3)))
		{
			spin_lock_irqsave(&ssp_lock,flags);
			in_cont = ssp_i2s.in_cont;
			spin_unlock_irqrestore(&ssp_lock, flags);
		...
		spin_lock_irqsave(&ssp_lock,flags);
		in_cont = ssp_i2s.in_cont;
		data_format = ssp_i2s.data_format;
		spin_unlock_irqrestore(&ssp_lock, flags);
		if ((LLP_SIZE - in_cont) <= 0)
			continue;
		spin_lock_irqsave(&ssp_lock,flags);
		wt_curr = ssp_i2s.wt_curr;
		spin_unlock_irqrestore(&ssp_lock, flags);
		...
		if (proc_w >= SBUF_SIZE)
		{
			...
			tmp_ptr = __va(ssp_i2s.LLPT[wt_curr].src_addr); 
			if (data_format == SSP_DF_16BITL_LINEAR || data_format == SSP_DF_16BITUL_LINEAR)
			{
			...
			ssp_i2s.LLPT[wt_curr].ctrl_size = (SBUF_SIZE/4)|0x040a<<16;
			len += SBUF_SIZE;
			proc_w -= SBUF_SIZE;
		}
		else if ((proc_w < SBUF_SIZE) && (proc_w > 0))
		{
			...
			ssp_i2s.LLPT[wt_curr].ctrl_size = (proc_w/4)|0x040a<<16;
			len += ofs;
			proc_w -= proc_w;
		}
		spin_lock_irqsave(&ssp_lock,flags);
		...
		in_bufok = ssp_i2s.in_bufok;
		wt_curr = ssp_i2s.wt_curr;
		spin_unlock_irqrestore(&ssp_lock, flags);
							
		if ((wt_curr > (LLP_SIZE/32))&&(in_bufok == 0))
		{
			spin_lock_irqsave(&ssp_lock,flags);
			ssp_i2s.in_bufok = 1;
		...
		if (proc_w >= SBUF_SIZE)
		{
			...
			ssp_i2s.LLPT[wt_curr].ctrl_size = (SBUF_SIZE/4)/2)|0x040a<<16;
			len += SBUF_SIZE;
			proc_w -= SBUF_SIZE;
		}
		else
		{
			...
			for (j = 0; j < ((proc_w+1)/2); j++)
			{
				*(tmp_ptr+j) = *(tmp+(2*j)+1);
				...
				ssp_i2s.LLPT[wt_curr].ctrl_size = (proc_w/4)/2)|0x040a<<16;
				len += ofs;
				proc_w -= proc_w;
		}
		spin_lock_irqsave(&ssp_lock,flags);
		...
		ssp_i2s.in_cont++;
		in_bufok = ssp_i2s.in_bufok;
		in_curr = ssp_i2s.in_curr;
		spin_unlock_irqrestore(&ssp_lock, flags);
					
		if ((in_curr == LLP_SIZE) && (in_bufok == 0))
		{
			spin_lock_irqsave(&ssp_lock,flags);
			ssp_i2s.in_bufok = 1;
			spin_unlock_irqrestore(&ssp_lock, flags);
...
static int gemini_i2s_dma(void)
{
	...
	*((volatile unsigned int *)IRQ_MASK(IO_ADDRESS(SL2312_INTERRUPT_BASE))) |= (unsigned int)(IRQ_DMA_MASK);
	mdelay(25);
	WRITE_DMA_REG(DMA_INT_TC_CLR, 0x4);
			
	//DMA Channel 2 & 3 int mask
	WRITE_DMA_REG(DMA_CH2_CFG, 0);
	data = READ_DMA_REG(DMA_CFG);
	if ((data & 0x1) != 0x1)
		WRITE_DMA_REG(DMA_CFG, 0x1); //enable DMA
	WRITE_DMA_REG(DMA_SYNC, 0x4);
	data = READ_GLOBAL_REG(0x30);
	tmp = READ_SSP_REG(SSP_CTRL_STATUS);
	printk("30(%x) 04(%x)\n",data,tmp);
	...
	WRITE_SSP_REG(SSP_CTRL_STATUS, 0xC00001FF);//0x9F100000
	printk("%s :<--\n",__func__);
...
int __init init_storm_ssp(void)
{
	...
	printk("Cortina I2S of SSP Initialization(dsp)\n");
...
void __exit cleanup_storm_ssp(void)
{
	...
	printk("Cortina I2S of SSP cleanup(dsp)\n");
