...
// return status
#define CRYPTO_COMPLETE		0
#define CRYPTO_QUEUE_EMPTY	1
#define CRYPTO_MISMATCH		2
#define CRYPTO_RESET		3

// Descriptor Queue Size + Buffer Size
#define     TX_BUF_SIZE				2048
#define     IPSEC_TX_DESC_NUM		128	//40	//20
#define     RX_BUF_SIZE				2048	//2048		//32768	//2048
#define     IPSEC_RX_DESC_NUM		64	//20	//8
...
struct IPSEC_PACKET_S
{
    ...
    unsigned char   *in_packet2;        /* input_packet buffer pointer type 2 */
    unsigned int    pkt_len;            /* input total packet length */
  //unsigned char   auth_checkval[20];  /* Authentication check value/FCS check value */
    unsigned char   *auth_checkval;
    unsigned int    auth_check_len;     /* Number of 32-bits words to be checked or appended */
    ...
    unsigned char   *out_packet2;       /* output_packet buffer pointer type 2 */
	...
	// IPSEC VPN required information
	struct sk_buff	*input_skb;			/* for IPSEC VPN to hold the address of input skb */
	struct sk_buff	*output_skb;		/* for IPSEC VPN to hold the address of output skb */
	__u8	icv_full_len;
	__u8	icv_trunc_len;
	unsigned int	tunnel_ID;
	unsigned int	used;
	unsigned int	flag_polling;
...
typedef struct ipsec_tasklet_private {
        struct semaphore        sem;
        struct tasklet_struct   tasklet;
        volatile unsigned long  sched;
} IPSEC_TASKLET_INFO_T;
#endif
