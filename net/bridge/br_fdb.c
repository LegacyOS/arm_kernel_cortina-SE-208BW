 ...
 *
 *	$Id: br_fdb.c,v 1.4 2009/07/03 01:36:56 jason Exp $
 *
...
int br_fdb_check_eth0 (unsigned char *addr)
{
	struct net_bridge_fdb_entry *fdb;
	...
	if (fdb)
	{
		...
		{		
				...
				if (!has_expired(br, fdb) && \
					( (fdb->dst->dev->irq==IRQ_GMAC0)||(fdb->dst->dev->irq==IRQ_GMAC1)
#ifdef CONFIG_VLAN_8021Q
					|| ((fdb->dst->dev->hard_start_xmit == vlan_dev_hard_start_xmit) && \
					(VLAN_DEV_INFO(fdb->dst->dev)->real_dev->irq==IRQ_GMAC0||VLAN_DEV_INFO(fdb->dst->dev)->real_dev->irq==IRQ_GMAC1)
					)
#endif
				)){
					rcu_read_unlock();
