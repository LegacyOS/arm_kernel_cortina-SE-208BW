 ...
 *
 * Version:	$Id: route.c,v 1.5 2009/07/01 20:58:48 whsu Exp $
 *
 * Authors:	Ross Biro
...
static void rt_run_flush(unsigned long dummy)
{
	...
	/*
	 * a simple fix by Wen:
	 * instead of using spin_lock_bh, I use spin_lock_irqsave, to force 
	 * this function to be protected from interrupt, not just BH.
	 * Apparently, the new kernel reschedules this task if it's in interrupt.
	 * If anything goes wrong with this easy fix, we might want to apply 
	 * the new kernel solution to fix this.
	 */
	rt_deadline = 0;
	...
	for (i = rt_hash_mask; i >= 0; i--) {
		...
		spin_lock_irqsave(rt_hash_lock_addr(i), flags);
		rth = rt_hash_table[i].chain;
		if (rth)
			rt_hash_table[i].chain = NULL;
		//spin_unlock_bh(rt_hash_lock_addr(i));
		spin_unlock_irqrestore(rt_hash_lock_addr(i), flags);
				...
				u8 tos, struct net_device *dev, int iif, int oif)
{
	...
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
		...
		if (rth->fl.fl4_dst == daddr &&
			...
			rth->fl.fl4_tos == tos &&
			rth->u.dst.neighbour &&
			(rth->u.dst.dev->hard_start_xmit == vlan_dev_hard_start_xmit)  /* Maybe we can remove this line to fit all interface */
			) {
			rth->u.dst.lastuse = jiffies;
			dst_hold(&rth->u.dst);
			rth->u.dst.__use++;
...
/* Used by IPsec VPN Acceleration. Wen Hsu */
#ifdef CONFIG_SL351X_IPSEC
int sl351x_ipsec_route_cache(struct sk_buff *skb, u32 daddr, u32 saddr,
				u8 tos)
{
	struct rtable *rth;
	unsigned hash;
