...
extern void conntrack_del_hash_entry(int port, u32 srcip, u32 dstip, u16 proto, u32 sport, u32 dport);
...
{
	unsigned int ho, hr;
	struct ip_conntrack_tuple *tuple_original, *tuple_reply;
	...
	tuple_original = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
	tuple_reply = &ct->tuplehash[IP_CT_DIR_REPLY].tuple;
	state = ct->proto.tcp.state;
	if (state == TCP_CONNTRACK_ESTABLISHED)
		{
			DEBUGP("original state = %d, ct->port= %d, src=%u.%u.%u.%u:%hu dst=%u.%u.%u.%u:%hu \n",
			state, !ct->port,
			NIPQUAD(tuple_original->src.ip), ntohs(tuple_original->src.u.tcp.port),
			NIPQUAD(tuple_original->dst.ip), ntohs(tuple_original->dst.u.tcp.port));	
			conntrack_del_hash_entry(!ct->port, ntohl(tuple_original->src.ip), ntohl(tuple_original->dst.ip),
				tuple_original->dst.protonum, tuple_original->src.u.tcp.port,
				tuple_original->dst.u.tcp.port);
			
			DEBUGP("reply state = %d, ct->port= %d, src=%u.%u.%u.%u:%hu dst=%u.%u.%u.%u:%hu \n",
			state, ct->port,
			NIPQUAD(tuple_reply->src.ip), ntohs(tuple_reply->src.u.tcp.port),
			NIPQUAD(tuple_reply->dst.ip), ntohs(tuple_reply->dst.u.tcp.port));	
			conntrack_del_hash_entry(ct->port, ntohl(tuple_reply->src.ip), ntohl(tuple_reply->dst.ip),
				tuple_reply->dst.protonum, tuple_reply->src.u.tcp.port,
				tuple_reply->dst.u.tcp.port);
	}
