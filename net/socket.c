...
//debug_Aaron on 07/06/2009, add ftpFinFlag for FTP finished indication
static int sock_recvpage(struct file *file, char* buf,
			     size_t size, loff_t *ppos, int more, int *ftpFinFlag);
			...
			size_t size, loff_t *ppos, int more, int *ftpFinFlag) 
{
	...
	return sock->ops->recvpage(sock, buf, size, file->f_flags & O_NONBLOCK, flags, ftpFinFlag);
}
#endif
