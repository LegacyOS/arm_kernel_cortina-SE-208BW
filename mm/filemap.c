	...
	int ftpFlag)
{
	...
	//debug_Aaron on 12/02/2008 save for laster use
	int total_count = (int) count, save_total_count = count;
	int tmp_cnt = 0;
	...
	//debug_Aaron on 07/02/2009
        if (storlink_ctl.recvfile==101)
        {
        	printk("%s: written=%d\r\n", __func__, written);
        }	
	...
	else
	{
		...
		while (1) 
		{
            ...
            if (storlink_ctl.recvfile==101)
            {
            	printk("%s: offset =%d, tmp_cnt=%d, written=%d\r\n", __func__, offset, tmp_cnt, written);
            }
			//debug_Aaron on 07/23/2009, save the page data to avoid data overwritten by a_ops->prepare_write()
			if (offset > 0 || (offset+tmp_cnt < PAGE_CACHE_SIZE))
			{
				
                //debug_Aaron on 07/02/2009
        		if (storlink_ctl.recvfile==101)
        		{
					printk("%s: offset > 0 OR offset+tmp_cnt < PAGE_CACHE_SIZE!!\r\n", __func__);
				}
				save_buf = kmalloc(PAGE_CACHE_SIZE, GFP_KERNEL);
				if (save_buf == NULL)
					printk("%s: Unable to allocate save buffer to save page data!!!\r\n", __func__);
				else
				{
            		kaddr = kmap_atomic(chain->page, KM_USER0);
					memmove(save_buf, kaddr+offset, tmp_cnt);
					kunmap_atomic(kaddr, KM_USER0);
				}
			}
            ...
            if (unlikely(status))
            {
				//debug_Aaron on 11/28/2008 for disk full issue, no need to do vmtruncate, PC will send message
                // to samba to SET_FILE_INFO, delete on close
            ...
            if (offset > 0 || (offset+tmp_cnt < PAGE_CACHE_SIZE))
			{
                ...
                {
                    kaddr = kmap_atomic(chain->page, KM_USER0);
                    memmove(kaddr+offset, save_buf, tmp_cnt);
					flush_dcache_page(chain->page);
                	kunmap_atomic(kaddr, KM_USER0);
					kfree(save_buf);
                }
			...
			if (status < 0)
            {
				...
				//debug_Aaron on 11/20/2008 to fix hard disk full wait a long time bug
                //status = -EFAULT;
...
fin_ok:  		// Zachary
	...
	*ppos = pos;
	...
	//debug_Aaron on 12/02/2008 if disk full have to read out all of SMB data packets
 	if (status < 0)
        {
                if (save_total_count > sock_data_len)
                {
                        total_count = save_total_count - sock_data_len;
                        start = chain = grab_write_pages(mapping, index, write_file, offset, total_count, &lru_pvec, &cached_page);
                        //debug_Aaron
                        printk("%s: disk full!!, start=0x%x, total_count=%d\r\n", __func__, start, total_count);
                        if (!chain)
                        {
                                //debug_Aaron
                                printk("%s: grab_write_pages(offset=%d, totoal_count=%d) return NULL !!!\r\n",
                        ...
                        {
				int try = 0;
                                while (total_count > 0 && try < 10)
                                {
                                        written = sock_file->f_op->recv_mpages(sock_file, chain, offset, total_count, ppos,
                                                                        1, &ftpFin);  // Zachary;

                                        //debug_Aaron
                                        printk("%s: disk full, written=%d\r\n", __func__, written);
                                        
                                        if (written <= 0)
                                        {       
                                                printk("%s: sock_file->f_op->recv_mpages() failed !!\r\n", __func__);
                                                break;
                                        }       
                                        total_count -= written;
					try++;
