...
#ifndef	__DISABLE_ASTEL_LT_485__
#include <linux/pci.h>
#endif
...
#define	RS485_RECIEVE_MODE			0
#define	RS485_TRANSMIT_MODE			1
#define	RS485_TRANSMIT_ECHO_MODE	2
#define RS485_DIRECTION_SET_REG		0x5c008048

static struct timer_list rs485Timeout = { function: NULL };
inline void set_rs485_mode(struct uart_port *, int, int);

extern int astel_serial_RS485;

#define	CORTINA_GPIO_0_DATA_OUT		(IO_ADDRESS(SL2312_GPIO_BASE) + 0x00)
#define	CORTINA_GPIO_0_DATA_IN		(IO_ADDRESS(SL2312_GPIO_BASE) + 0x04)
#define	CORTINA_GPIO_0_DIR			(IO_ADDRESS(SL2312_GPIO_BASE) + 0x08)
#define	CORTINA_GPIO_0_BYPASS		(IO_ADDRESS(SL2312_GPIO_BASE) + 0x0C)
#define	CORTINA_GPIO_0_PULL_EN		(IO_ADDRESS(SL2312_GPIO_BASE) + 0x18)
#define	CORTINA_GPIO_0_PULL_TYPE	(IO_ADDRESS(SL2312_GPIO_BASE) + 0x1C)
#define	CORTINA_GPIO_0_INTR_EN		(IO_ADDRESS(SL2312_GPIO_BASE) + 0x20)

#define	ASTEL_RS485_CPUGPIO_RE		0x02
#define	ASTEL_RS485_CPUGPIO_DE		0x10

#define	ASTEL_RS485_CPUGPIO			(ASTEL_RS485_CPUGPIO_RE | ASTEL_RS485_CPUGPIO_DE)

static void *astelAsicRs485Gpio = NULL;
static int astelRs485GpioInitDone = 0;

#define PCI_CONF_ASTEL				0x9229                  // 0x9229(LT)
#define PCI_CONF_ASTEL_LT_1			0x6ff4
static struct pci_dev *ast1000PciDev = NULL;

static void
_initCpuRs485Gpio(void)
{
	int value;

	// 1. GPIO Dir : 0=input
	value = readl(CORTINA_GPIO_0_DIR);
	value |= ASTEL_RS485_CPUGPIO;
	writel(value, CORTINA_GPIO_0_DIR);

	// 2. GPIO Bypass : 0=no bypass
	value = readl(CORTINA_GPIO_0_BYPASS);
	value &= ~ASTEL_RS485_CPUGPIO;
	writel(value, CORTINA_GPIO_0_BYPASS);

	// 3. GPIO Intr Enable : 0=disable
	value = readl(CORTINA_GPIO_0_INTR_EN);
	value &= ~ASTEL_RS485_CPUGPIO;
	writel(value, CORTINA_GPIO_0_INTR_EN);

	// 4. GPIO Pull Type : 1=pull high
	value = readl(CORTINA_GPIO_0_PULL_TYPE);
	value &= ~ASTEL_RS485_CPUGPIO;
	writel(value, CORTINA_GPIO_0_PULL_TYPE);

	// 5. GPIO Pull Enable : 1=pull enable
	value = readl(CORTINA_GPIO_0_PULL_EN);
	value &= ~ASTEL_RS485_CPUGPIO;
	writel(value, CORTINA_GPIO_0_PULL_EN);
}

static void
_initRs485GpioInfo(void)
{
	u8 read_val;

	if (astelRs485GpioInitDone)
		return;

#ifdef	__NOT_USED__
	if (astelAsicRs485Gpio == NULL)
	  {
		if (ast1000PciDev == NULL)
			ast1000PciDev = pci_get_device(PCI_CONF_ASTEL, PCI_CONF_ASTEL_LT_1, NULL);

		if (ast1000PciDev != NULL)
		  {
			pci_read_config_byte(ast1000PciDev, 0x40, &read_val);
			if( read_val != 0 )
			  {
				astelAsicRs485Gpio = ioremap_nocache(RS485_DIRECTION_SET_REG, 8);
				astelRs485GpioInitDone++;
			  }
		  }
	  }
#else
	...
	/*	driver 삽입이 늦어질 수 있으므로 ....
	 *	astelRs485GpioInitDone++;
	 */
}

static void
reset_rs485_transmit_mode(unsigned long data)
{
	struct uart_port *info = (struct uart_port *)data;
	int status;

	status = UART_GET_LSR(info);

	if (status & UART_LSR_TE)
	  {
		del_timer(&rs485Timeout);

		rs485Timeout.function = NULL;
		rs485Timeout.expires  = 0;

		set_rs485_mode(info, RS485_RECIEVE_MODE, 8);
	  }
	else
		mod_timer(&rs485Timeout, jiffies + HZ);
}

static void
reset_rs485_timeout(struct uart_port *info)
{
	if (rs485Timeout.function == NULL)
	  {
		init_timer(&rs485Timeout);

		rs485Timeout.function = reset_rs485_transmit_mode;
		rs485Timeout.expires  = jiffies + HZ;
		rs485Timeout.data = (unsigned long)info;

		add_timer(&rs485Timeout);
	  }
	else
		mod_timer(&rs485Timeout, jiffies + HZ);
}
...
inline void
set_rs485_mode(struct uart_port *info, int mode, int pos)
{
	static int rs485_mode = -1;
	unsigned int asicGpio = 0;
	unsigned int cpuGpio = 0;
	...
	/*	H/W 구성안
	 *
	 *		b1 b0		<- RS485_DIRECTION_SET_REG
	 *		 |  |
	 *		 |  +--------- Transmit Mode(DE-485) : 0=Disable, 1=Enable	
	 *		 +------------ Receive Mode(RE-485)  : 0=Enable,  1=Disable
	 *
	 *		TX Only : 0x03
	 *		RX Only : 0x00
	 */

	if (astelRs485GpioInitDone == 0)
		_initRs485GpioInfo();
	
	if (astel_serial_RS485 == 0)		// RS232 Mode
		return;

	if (astelAsicRs485Gpio)
	  {
		asicGpio  = readl(astelAsicRs485Gpio);
		asicGpio &= ~0x03;
	  }

	cpuGpio  = readl(CORTINA_GPIO_0_DATA_IN);
	cpuGpio &= ~ASTEL_RS485_CPUGPIO;

	switch (mode)
	  {
		case RS485_TRANSMIT_ECHO_MODE:
		case RS485_RECIEVE_MODE:
		default:
			if (rs485_mode != RS485_RECIEVE_MODE)
			  {
				rs485_mode = RS485_RECIEVE_MODE;

				if (astelAsicRs485Gpio)
					writel(asicGpio, astelAsicRs485Gpio);

				writel(cpuGpio, CORTINA_GPIO_0_DATA_OUT);
			  }
			break;

		case RS485_TRANSMIT_MODE:
			if (rs485_mode != RS485_TRANSMIT_MODE)
			  {
				rs485_mode = RS485_TRANSMIT_MODE;
				...
				reset_rs485_timeout(info);
			  }
			break;
	  }
}

#endif		// __DISABLE_ASTEL_LT_485__


static void sl2312_stop_tx(struct uart_port *port, u_int from_tty)
{
	unsigned int reg;
...
#ifndef	__DISABLE_ASTEL_LT_485__
	/*	여기서 Receive Mode로 변경하면 Tx Buffer에 있는 data가 보내지지 않는다.
	 *	Tx Timeout에 의하여 Receive Mode로 변경하도록 한다.
	 */
	// set_rs485_mode(port, RS485_RECIEVE_MODE, 4);
#endif
}
...
static void
sl2312_rx_chars(struct uart_port *port, struct pt_regs *regs)
{
...
static void sl2312_tx_chars(struct uart_port *port)
{
	...
	{
#ifndef	__DISABLE_ASTEL_LT_485__
		set_rs485_mode(port, RS485_TRANSMIT_MODE, 2);
#endif
	...
	{
		sl2312_stop_tx(port, 0);
		return;
	}

	count = port->fifosize >> 1;
#ifndef	__DISABLE_ASTEL_LT_485__
	set_rs485_mode(port, RS485_TRANSMIT_MODE, 3);
#endif
	do {
		while(!(UART_GET_LSR(port)&UART_LSR_THRE))
			;
...
static void sl2312_start_tx(struct uart_port *port, unsigned int tty_start)
{
	unsigned int reg;
...
static irqreturn_t sl2312_int(int irq, void *dev_id, struct pt_regs *regs)
{
...
static void sl2312_set_termios(struct uart_port *port, struct termios *termios,
                               struct termios *old)
{
...
static int sl2312_startup(struct uart_port *port)
{
...
#ifndef __DISABLE_ASTEL_LT_485__
	unsigned int cpuGpio;

	/* LT 만 처리해준다.
	 * UART가 하나밖에 없으므로 호환해주기 위해..
	 */
	if (astelRs485GpioInitDone == 0)
		_initRs485GpioInfo();
	...
	if (astel_serial_RS485 == 0)		// RS232 Mode
	  {
		...
		if (astelAsicRs485Gpio)
		  {
			regs |= 0x02;
			writel(regs, astelAsicRs485Gpio);
		  }

		cpuGpio |= ASTEL_RS485_CPUGPIO_RE;
		writel(cpuGpio, CORTINA_GPIO_0_DATA_OUT);
	  }
	else		// RS485 Mode
	  {
	 ...
	 * Allocate the IRQ
	 */
	retval = request_irq(port->irq, sl2312_int, SA_INTERRUPT, "sl2312", port);
	if (retval)
		return retval;
...
#ifndef	__DISABLE_ASTEL_LT_485__
	if (status & UART_LSR_TE)
		set_rs485_mode(port, RS485_RECIEVE_MODE, 5);
#endif
}
