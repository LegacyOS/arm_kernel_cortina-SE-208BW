...
static int hid_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	//////////////////////// ASTEL
	// USB Device Name Check - dywon
	// printk("############################### hid_device Name: %s \n", hid->name);

	// CHECKCHECKCHECKCHECK  dhsul
	// if(Touch Screen) return -ENODEV;
	// printk("############################### hid_probe: just return with -ENODEV \n");

	for(i = 0; i < strlen(hid->name); i++)
		if(hid->name[i] >= 'A' && hid->name[i] <= 'Z')	
			hid->name[i] = hid->name[i] + ('a' - 'A');
	if(strstr(hid->name, "touch") != NULL)
		return -ENODEV;
	//////////////////////// ASTEL
