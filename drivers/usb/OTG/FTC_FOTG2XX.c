...
#define CPE_FOTG200_BASE                        0x68000000//0x68000000
#define IRQ_FOTG200                                IRQ_USB0//IRQ_USB0
...
static void OTGC_enable_vbus_draw_storlink(u8 btype);
...
//extern int USB_driver_check(void);
...
extern wait_queue_head_t usb_wait_q;
...
//            interruptible_sleep_on(&usb_wait_q);
//            printk("**************** USB_driver_flag %x ********************\n",USB_driver_Flag);
            return USB_driver_Flag;
...
void OTGC_Init(void)
{
  ...
  pFTC_OTG->otg.host->A_Disable_Set_Feature_HNP=1;
      ...
      if(pFTC_OTG->got_irq != 1) //stone Host interrupt always up !
        {
          if(request_irq(IRQ_FOTG200, FOTG2XX_ISR, SA_INTERRUPT,driver_name, pFTC_OTG) != 0) 
	    {
	    	printk("OTGC_Init request irq fail !!!!\n");
...
#ifdef	DEBUG_ASTEL
                    printk("pFTC_OTG->otg.default_a=1 \n");
#endif
...
static void FOTG2XX_Remove(void)
{
   ...
   pFTC_OTG = 0;
   unregister_chrdev(OTG_MAJOR, "FOTG2XX");
   wINTStatus = mdwOTGC_INT_STS_Rd(); 
   mdwOTGC_INT_STS_Clr(wINTStatus);
...
// Output: int          
//********************************************************* 
static int FOTG2XX_Probe_storlink(void)//FOTG200.ok
{
    int	retval=0;
    u8 wFound=0;
...
//	if (devfs_register_chrdev(OTG_MAJOR, "FOTG2XX", &otg_fops)) {
	if (register_chrdev(OTG_MAJOR, "FOTG2XX", &otg_fops)) {
...
//    pFTC_OTG->iTMP_Force_Speed=2;//Default force speed to Full
    pFTC_OTG->iTMP_Force_Speed=0;//Default force speed to Full for new PHY will auto detect speed!!

    init_waitqueue_head(&usb_wait_q);
