...
#if !defined  (CONFIG_USB_SL2312) && !defined (CONFIG_USB_SL2312_MODULE) //######################### For FEHCI
...
#if defined  (CONFIG_USB_SL2312) || defined (CONFIG_USB_SL2312_MODULE)//John_Update
...
#if defined  (CONFIG_USB_SL2312) || defined (CONFIG_USB_SL2312_MODULE) //Bruce;;   
        int     val=0;     
#endif
		...
		if ((temp & PORT_RESET)&& time_after (jiffies,ehci->reset_done [wIndex]))
		 {
			...
			timeout = 0;	// wait 0.1 sec
			do { //stone 2.6.15
				temp = readl (&ehci->regs->port_status [wIndex]);
				udelay (10);
				timeout++;
			} while ((temp & PORT_RESET) && (timeout<10000));
...
#else
//     DBG_HOST_TEMP("### ehci-hub.c-->ehci_hub_control()-->Detect the Speed ...\n");    
                      if (temp & PORT_CONNECT) 
                         {
                           status |= 1 << USB_PORT_FEAT_CONNECTION;
