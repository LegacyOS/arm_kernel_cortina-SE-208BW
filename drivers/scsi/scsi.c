...
#define	ASTEL_SATA_LED				0x00001000		// GPIO0-12

static void
_setAstelSataLED(int on)
{
	static unsigned int _gpio_init = 0;
	int value;
	...
	value = readl(CORTINA_GPIO_0_DATA_IN);

	if (on == 0)
	  {
		if (value & ASTEL_SATA_LED)
		  {
			value &= ~ASTEL_SATA_LED;
			writel(value, CORTINA_GPIO_0_DATA_OUT);
		  }
	  }
	else
	  {
		if ((value & ASTEL_SATA_LED) == 0)
		  {
			value |= ASTEL_SATA_LED;
			writel(value, CORTINA_GPIO_0_DATA_OUT);
		  }
...
int
_astel_sata_led(struct scsi_cmnd *cmd, int add)
{
	struct scsi_host_template *hostt;
	unsigned int proc_id0;
	unsigned int proc_id1;
	...
	if (cmd == NULL
	 ...
	 || cmd->device->host->hostt == NULL)
	  {
		return (_max << 16) | (_cnt & 0xffff);
	  }

	if (add < 0)
		return (_max << 16) | (_cnt & 0xffff);

	hostt = cmd->device->host->hostt;

	proc_id0 = *(unsigned int *)&hostt->proc_name[0];
	proc_id1 = *(unsigned int *)&hostt->proc_name[4];

	/*  DT: hostt->proc_name == "mvSata"   : ['a''S''v''m']==0x6153766d
	 *      hostt->this_id   == 0x0f
	 *  MT: hostt->proc_name == "sata_fsl" : ['s''a''t''a']==0x73617461 ['_''f''s''l']==0x5f66736c
	 *                          "sata_sil" : ['s''a''t''a']==0x73617461 ['_''s''i''l']==0x5f73696c
	 *      hostt->this_id   == 0xffffffff
	 *  LT: hostt->proc_name == "sata_lepus" : ['a''t''a''s']==0x61746173 ['p''e''l''_']==0x70656c5f
	 */

	if (_init == 0)
	  {
		if (proc_id0 == 0x61746173)								// "sata"
		  {
			printk("=================================\n");
			printk("1. proc_name=[%s]\n", hostt->proc_name);
			printk("2. this_id=0x%x\n", hostt->this_id);
			printk("3. proc_id0=0x%x\n", proc_id0);
			printk("3. proc_id1=0x%x\n", proc_id1);
			printk("4. queuecommand=0x%x\n", (int)(hostt->queuecommand));
			printk("=================================\n");
			_init++;
		  }
	  }

	if (proc_id0 != 0x61746173)
		return (_max << 16) | (_cnt & 0xffff);
...
int scsi_dispatch_cmd(struct scsi_cmnd *cmd)
{
	...
	if (unlikely(host->shost_state == SHOST_DEL)) {
		...
		rtn = host->hostt->queuecommand(cmd, scsi_done);	// _astel_sata_led: ata_scsi_queuecmd()
	}
...
void __scsi_done(struct scsi_cmnd *cmd)
{
	...
	/*	정상 종료 or timeout등으로 호출되는 위치이다.
	 */
	_astel_sata_led(cmd, 0);
}
