...
EXPORT_SYMBOL(sl2312flash_enable_parallel_flash);
...
EXPORT_SYMBOL(sl2312flash_disable_parallel_flash);
#endif
...
static int __init sl2312flash_init(void)
{
    ...
    printk("SL35XX MTD Driver Init.......\n");
#ifdef CONFIG_SL2312_SHARE_PIN
    sl2312flash_enable_parallel_flash();      /* enable Parallel FLASH */
#else
	//debug_Aaron on 08/30/2007 enable flash and disable IDE always
        unsigned int value;
   	...
   	printk("%s: value=0x%x\r\n", __func__, value);
    	...
    	value &= 0xffffffef;
   	printk("%s: value=0x%x after mask\r\n", __func__, value);
...
//	simple_map_init(&sl2312flash_map);
	...
	mtd = do_map_probe("jedec_probe", &sl2312flash_map);
#else
...
//    mtd->write = flash_write;
    ...
    sl2312flash_disable_parallel_flash();      /* disable Parallel FLASH */
#endif
    printk("SL35XX MTD Driver Init Success ......\n");
	return ret;
}
...
int get_vlaninfo(vlaninfo* vlan)
{
	...
	int i, j, loc = 0;
	char *payload=0, *tmp1, *tmp2, *tmp4,tmp3[9];
	size_t retlen;
...
#ifdef CONFIG_RTL8366SR_PHY
				vlan[2].mac[j] = chrtohex(tmp4[2*j])*16 + chrtohex(tmp4[(2*j)+1]);
#endif
			}
			tmp1 = strstr(payload, "ID1:");
			tmp2 = strstr(payload, "ID2:");
#ifdef CONFIG_RTL8366SR_PHY
			tmp4 = strstr(payload, "ID3:");
#endif
			tmp1 += 4;
			tmp2 += 4;
#ifdef CONFIG_RTL8366SR_PHY
...
#ifdef CONFIG_RTL8366SR_PHY
			vlan[2].vlanmap = chrtohex(tmp4[0]) * 16 + chrtohex(tmp4[1]);
#endif
