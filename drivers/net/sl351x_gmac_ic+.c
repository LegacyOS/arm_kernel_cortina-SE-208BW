...
#ifdef DUAL_BAND_VIRTUAL_INTERFACE
static int gmac0_if_running(void);
static int gmac0_vif1_open(struct net_device *dev);
static int gmac0_vif1_close(struct net_device *dev);
static int gmac0_set_if_active_flag(struct net_device *dev, int flag) ;
#endif	
...
//	init_waitqueue_entry(&wait, current);
	...
	{	
		...
		if(i == 1)
        {
	        dev->open = gmac0_vif1_open;
	        dev->stop = gmac0_vif1_close;       
        }
        else
        {
	        dev->open = gmac_open;
...
// Modified by Andrew,kim on 2011,10-7 for 10/100 IC+ IP101A 
#ifdef CONFIG_SL3516_ASIC
       ...
       // toe->gmac[0].phy_mode = GMAC_PHY_RGMII_1000;
        ...
        toe->gmac[0].phy_mode = GMAC_PHY_MII;
...
//	unsigned int			tx_qid;
	...
	{
#ifdef DUAL_BAND_VIRTUAL_INTERFACE		
		if((strstr(dev->name,"eth0")) || (strstr(dev->name,"eth1")))
#endif		
		{
...
#ifdef DUAL_BAND_VIRTUAL_INTERFACE					
 				if (strstr(dev->name,"eth0"))	//to lan interface
 				{
 					veth_TCI = 0x2;
 					tp = VLAN_conf[0].dev->priv;
 				}
 				if (strstr(dev->name,"eth1"))	//to dmz interface
 				{
 					veth_TCI = 0x3;
 					tp = VLAN_conf[1].dev->priv;
 				}
#else 				
				veth_TCI = 0x2; 				
#endif	 		
...
//#if (GMAC_DEBUG==1)
...
#else
		if ((tp->port_id == 0) && total_len <= 1518 && skb->nh.iph && (skb->nh.iph->frag_off & __constant_htons(0x3fff)))		
			word1  = total_len | TSS_IP_CHKSUM_BIT  | TSS_IPV6_ENABLE_BIT | TSS_MTU_ENABLE_BIT;
		
		else if ((tp->port_id == 1) && total_len <= 1514 && skb->nh.iph && (skb->nh.iph->frag_off & __constant_htons(0x3fff)))	
#endif					
...
#ifdef CONFIG_SL351x_NAT
#ifdef SKIP_NAT_CFG	
				nat_cfg.enabled = 1;	//temp add,must remove
#endif
		...
		if (gmac_dump_txpkt)
		{
			printk("******** GMAC %d Tx (Size = %d): skb->data = %p ********\n",tp->port_id, total_len, skb->data);
//			 dm_byte((u32)skb->data, (total_len > 256) ? 256 : total_len)
...
static int gmac_set_mac_address(struct net_device *dev, void *addr)
{
	...
	if(tp->port_id==0){//GMAC0
		...
		if(strstr(dev->name,"eth1")){
			for (i = 0; i < 6; i++)
			{
				if(i==5)
					dev->dev_addr[i] = sock->sa_data[i]+2;
				else
					dev->dev_addr[i] = sock->sa_data[i];
			}
    ...
    else
    {
	    ...
	    reg_val = dev->dev_addr[2] + (dev->dev_addr[3]<<8) + (dev->dev_addr[4]<<16) + (dev->dev_addr[5]<<24);
        gmac_write_reg(tp->base_addr, GMAC_STA_ADD2,reg_val,0xffffffff);
        memcpy(&eth_mac[1][0],&dev->dev_addr[0],6);
    }
#endif    
#else 
    reg_val = dev->dev_addr[0] + (dev->dev_addr[1]<<8) + (dev->dev_addr[2]<<16) + (dev->dev_addr[3]<<24);
...
static int gmac0_if_running(void)
{
    ...
    /* to check virtual interface is running */	
	for (i=1;i<2;i++) 
	{
		if( VLAN_conf[i].flag == ACTIVE )
		{
		    return (1);
		}    
	}
...
static int gmac0_vif1_open(struct net_device *dev)
{
    ...
    if (VLAN_conf[0].flag == NONACTIVE ) /* eth0 is not active */
    {
        printk("Unable to active the interface !\n");
        return 0;
    }
//     gmac_vlan = 1;
    memcpy(&sock.sa_data[0],&eth_mac[0],6);
//  	memcpy(&sock.sa_data[0],&eth_mac[1][0],6);//consider again
//  gmac0_set_mac(dev,(void *)&sock);
	gmac_set_mac_address(dev,(void *)&sock);
	...
	dev->dev_addr[5]=0x66;
#endif	
    gmac0_set_if_active_flag(dev,ACTIVE);
//    printk("gmac0_vif1_open -- DONE !\n");
	return 0;    
}
/*----------------------------------------------------------------------
* gmac0_vif1_close
...
static int gmac0_vif1_close (struct net_device *dev) 
{
    /* set net device status to non-active */
    gmac0_set_if_active_flag(dev,NONACTIVE);
    return 0;
}    
/*----------------------------------------------------------------------
* gmac0_set_if_active_flag
*----------------------------------------------------------------------*/
static int gmac0_set_if_active_flag(struct net_device *dev, int flag) 
{
	int i;
	...
	for (i=0;i<2;i++) 
	{
		if( dev == VLAN_conf[i].dev )
		{
		    VLAN_conf[i].flag = flag;
		    break;
		}    
	}
	return 0;
...
#ifdef DUAL_BAND_VIRTUAL_INTERFACE	//debug purpose
	/* if eth0/1 is active,don't open eth0/1 twice */
	if(dev == toe->gmac[0].dev)
	{
	    if(gmac0_if_running())
            return 0;
	}
...
#ifdef CONFIG_RTL8366SR_PHY
	if((tp->port_id == GMAC_PORT0)&&(Giga_switch==1)){
...
// Modify by Andrew,kim on 2011-10-7 to support IC+ IP101A 10/100 PHY
#if 0
...
#endif	
#ifdef  CONFIG_RTL8366SR_PHY
	get_link = rtl8366sr_phy_is_up(0);
#else		
...
phy_status_down:	
   ...
   // Added by Andrew,kim on 2011-10-7 for IC+ IP101A 10/100 PHY
...
// Modified by Andrew,kim on 2011-10-7 for IC+ IP101a PHY , MDC=GPIO0[21] , MDIO=GPIO0[22] 
#define H_MDC_PIN           21
		...
		{	       
				...
				|| rx_status )
			{
				...
				if (gmac_dump_rxpkt)
				{
					...
					printk("Checksum Status=%d, L3:0x%x, L4:0x%x, L7:0x%x skb->data = %p\n", 
...
#ifdef DUAL_BAND_VIRTUAL_INTERFACE				
					if(vid_skb == 0x2002)
                      skb->dev = VLAN_conf[0].dev;	//if incoming packet from LAN
					else
				      skb->dev = VLAN_conf[1].dev;  //if incoming packet from DMZ   
#else
					skb->dev = dev;	
#endif		   		     
   		    		for(si = 0; si < 3; si++)
