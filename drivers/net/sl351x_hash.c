		...
		if (own_bits)
		{
			for (j=0; own_bits && j<32; j++)
			{
				if (own_bits & 1)
				{
					...
					igmp_entry = (NAT_0_HASH_ENTRY_T *)nat_entry;
					...
					if (nat_entry->key.ip_protocol == IPPROTO_GRE)
					{
						...
						counter_p = (short *)&ipsec_entry->tmo.counter;
						interval_p = (short *)&ipsec_entry->tmo.interval;
					} else if (igmp_entry->key.da[0] == 0x01) {
						counter_p = (short *)&igmp_entry->tmo.counter;
						interval_p = (short *)&igmp_entry->tmo.interval;
					} else {
