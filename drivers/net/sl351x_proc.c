...
static int nat_ct_seq_show(struct seq_file *s, void *v)
{
	...
	NAT_0_HASH_ENTRY_T	*igmp_entry;
	IPSEC_HASH_ENTRY_T	*ipsec_entry;
	...
	igmp_entry = (NAT_0_HASH_ENTRY_T *)&hash_tables[idx];
	ipsec_entry = (IPSEC_HASH_ENTRY_T *)nat_entry;
	
	if ((nat_entry->key.ip_protocol == IPPROTO_ESP) || 
			(nat_entry->key.ip_protocol == IPPROTO_AH)) {
		c_qid = ipsec_entry->action.bits.dest_qid;
	} else
		c_qid = nat_entry->action.bits.dest_qid;
	ig_qid = igmp_entry->action.bits.dest_qid;
	switch (c_qid)	
	{
		case TOE_GMAC0_HW_TXQ0_QID:	
			c_qid = 0;
			gmac_qid = 0;
			break;
		case TOE_GMAC0_HW_TXQ1_QID:	
			c_qid = 1;
			gmac_qid = 0;
			break;
		case TOE_GMAC0_HW_TXQ2_QID:	
			c_qid = 2;
			gmac_qid = 0;
			break;
		case TOE_GMAC0_HW_TXQ3_QID:	
			c_qid = 3;
			gmac_qid = 0;
			break; 
		case TOE_GMAC1_HW_TXQ0_QID:	
			c_qid = 0;
			gmac_qid = 1;
			break;
		case TOE_GMAC1_HW_TXQ1_QID:	
			c_qid = 1;
			gmac_qid = 1;
			break;
		case TOE_GMAC1_HW_TXQ2_QID:	
			c_qid = 2;
			gmac_qid = 1;
			break;
		case TOE_GMAC1_HW_TXQ3_QID:	
			c_qid = 3;
			gmac_qid = 1;
			break; 
	}
	...
	{
#if defined(CONFIG_SL351x_RTLDMZ) || defined(CONFIG_CS351X_DUAL_WAN)
		if (seq_printf(s, "%4d: KEY MAC-%d [%d] vlan=%d %u.%u.%u.%u [%u]-->%u.%u.%u.%u\n", 
					idx, gre_entry->key.port_id, gre_entry->key.ip_protocol,nat_entry->key.vlan_id,
					HIPQUAD(gre_entry->key.sip), ntohs(gre_entry->key.call_id),
					HIPQUAD(gre_entry->key.dip)))
			return -ENOSPC;
		if (seq_printf(s, "      PARAMETER: vlan=%d %u.%u.%u.%u -->%u.%u.%u.%u [%u] Timeout:%ds\n", 
					nat_entry->param.vlan,HIPQUAD(gre_entry->param.Sip),
	...
	else if ((nat_entry->key.ip_protocol == IPPROTO_TCP) || 
			(nat_entry->key.ip_protocol == IPPROTO_UDP))
	{
#if defined(CONFIG_SL351x_RTLDMZ) || defined(CONFIG_CS351X_DUAL_WAN)
		if (seq_printf(s, "%4d: KEY MAC-%d [%d] vlan=%d %u.%u.%u.%u [%u]-->%u.%u.%u.%u [%u]\n", 
					idx, nat_entry->key.port_id, nat_entry->key.ip_protocol,nat_entry->key.vlan_id,
					HIPQUAD(nat_entry->key.sip), ntohs(nat_entry->key.sport),
					HIPQUAD(nat_entry->key.dip), ntohs(nat_entry->key.dport)))
			return -ENOSPC;
		if ((nat_entry->param.Dip != 0) && (nat_entry->param.Sip != 0)) {
			if (seq_printf(s, "      PARAMETER: vlan=%d %u.%u.%u.%u [%u]-->%u.%u.%u.%u [%u] GMAC-%d HWTXQ-%d Timeout:%ds\n", 
					nat_entry->param.vlan,HIPQUAD(nat_entry->param.Sip), nat_entry->param.Sport,
					HIPQUAD(nat_entry->param.Dip), nat_entry->param.Dport, gmac_qid, c_qid, 
					nat_entry->tmo.counter))
				return -ENOSPC;
		} else {
			if (seq_printf(s, "      PARAMETER: HWTXQ-%d Timeout:%ds\n", c_qid,
					nat_entry->tmo.counter))
				return -ENOSPC;
		}
#else
		if (seq_printf(s, "%4d: KEY MAC-%d [%d] %u.%u.%u.%u [%u]-->%u.%u.%u.%u [%u]\n", 
			...
			return -ENOSPC;
		if ((nat_entry->param.Dip != 0) && (nat_entry->param.Sip != 0)) {
			if (seq_printf(s, "      PARAMETER: %u.%u.%u.%u [%u]-->%u.%u.%u.%u [%u] GMAC-%d HWTXQ-%d Timeout:%ds\n", 
	...
	else if (igmp_entry->key.da[0] == 0x1 && igmp_entry->key.da[2] == 0x5e) 	/* Multicast Packet */
	{	
		if (seq_printf(s, "%4d: KEY GMAC-%d DA: %X:%X:%X:%X:%X:%X [%d] DIP: %u.%u.%u.%u \n", 
						idx, igmp_entry->key.port_id,
						igmp_entry->key.da[0],igmp_entry->key.da[1],igmp_entry->key.da[2]
						,igmp_entry->key.da[3],igmp_entry->key.da[4],igmp_entry->key.da[5]
						,igmp_entry->key.ip_protocol, HIPQUAD(igmp_entry->key.dip)))
				return -ENOSPC;

#ifdef CONFIG_CS351X_DUAL_WAN				
		if (seq_printf(s, "      PARAMETER: GMAC-%d HWTXQ-%d vlan=%d Timeout: %d Second\n", 
						igmp_qid, ig_qid , igmp_entry->param.vlan, igmp_entry->tmo.counter))
				return -ENOSPC;		
#else
		if (seq_printf(s, "      PARAMETER: GMAC-%d HWTXQ-%d Timeout: %d Second\n", 
						igmp_qid, ig_qid , igmp_entry->tmo.counter))
				return -ENOSPC;	
#endif				
		
	}
#ifdef CONFIG_SL351X_IPSEC
	else if ((nat_entry->key.ip_protocol == IPPROTO_ESP) || 
			(nat_entry->key.ip_protocol == IPPROTO_AH)) {
#ifdef CONFIG_CS351X_DUAL_WAN
		if (seq_printf(s, "%4d: KEY MAC-%d [%d] vlan=%d %u.%u.%u.%u -->%u.%u.%u.%u\n", 
					idx, ipsec_entry->key.port_id, ipsec_entry->key.ip_protocol,ipsec_entry->key.vlan_id,
					HIPQUAD(ipsec_entry->key.sip), HIPQUAD(ipsec_entry->key.dip)))
			return -ENOSPC;
#else
		if (seq_printf(s, "%4d: KEY MAC-%d [%d] %u.%u.%u.%u -->%u.%u.%u.%u\n", 
					idx, ipsec_entry->key.port_id, ipsec_entry->key.ip_protocol,
					HIPQUAD(ipsec_entry->key.sip), HIPQUAD(ipsec_entry->key.dip)))
			return -ENOSPC;
#endif
		if (seq_printf(s, "      PARAMETER: HWTXQ-%d Timeout:%ds\n", c_qid,
					ipsec_entry->tmo.counter))
			return -ENOSPC;
	}
#endif
	return 0;
...
static int gmac_ct_seq_show(struct seq_file *s, void *v)
{
	switch ((int)v)
	{
		...
		case 4:
			...
			seq_dm_long(s, TOE_GMAC0_DMA_BASE, 60);
			break;
		case 5:
			seq_printf(s, "\nGMAC-0 Registers\n");
		...
		case 6:
			...
			seq_dm_long(s, TOE_GMAC1_DMA_BASE, 60);
			break;
		case 7:
			seq_printf(s, "\nGMAC-1 Registers\n");
