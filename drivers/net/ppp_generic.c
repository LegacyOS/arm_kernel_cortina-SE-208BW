...
//debug_Aaron on 01/07/2008 need 4 more bytes
	...
	/* Wen Debug on 02/18/2009.
	 * for some weird reasons, the size of IP "fragmented" packet going to 
	 * be compressed by mppe somehow increases by 2. check sum must be 
	 * recalculated before entering compression method. 
	 * this fix is not that clean, if we can find a better way to fix it, 
	 * we better change this fix
	 */
	if ((ethertype_to_npindex(ntohs(skb->protocol)) == NP_IP) && 
			(skb->nh.iph != NULL)) {
	//	printk("%s:kernel:going to re-calculate ip checksum\n", __func__);
