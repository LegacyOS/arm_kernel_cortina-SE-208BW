...
#ifndef    __DISABLE_ASTEL_TURNOFF_WINDOW__
#define    BUZZER_BASE         0x5c00804c
#define    BUZZER_SOUND_ON     0x1

static void 
machine_power_off_buzzer(int toggle)
{  
	int i;
	int gpio;
	void *base;

	base = ioremap_nocache(BUZZER_BASE, 8);
	gpio = readl(base);

	gpio |= BUZZER_SOUND_ON;
	writel(gpio, base);
...
typedef void (*vfuncPtr_t)(int);
static vfuncPtr_t turnOffPowerWindow = NULL;

void
registerTurnOffPowerWindow(int funcPtr)
{
	turnOffPowerWindow = (vfuncPtr_t)funcPtr;
}

/*  arch/arm/kernel/armksyms.c 에서 정의 하였음.
 *  EXPORT_SYMBOL(registerTurnOffPowerWindow);
 */
#endif
...
#else
#ifndef __DISABLE_ASTEL_TURNOFF_WINDOW__
    if (turnOffPowerWindow != NULL)
        (*turnOffPowerWindow)(1);

   machine_power_off_buzzer(1);
#endif
#endif
...
#ifndef __DISABLE_ASTEL_TURNOFF_WINDOW__
	if (turnOffPowerWindow != NULL)
		(*turnOffPowerWindow)(2);

	machine_power_off_buzzer(0);
#endif
